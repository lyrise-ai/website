...
import {
  ...
  expertsSection,
  aiSection,
} from '../src/constants/dataEngineers'
import LeaveToLyRise from '../src/components/LeaveToLyRise/LeaveToLyRise'

export default function DataEngineers() {
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (
      ...
      amplitude.getInstance().logEvent('visitedEmployerHomePage-DataEngineers')
      ReactGA.event({
        category: 'DataEngineers',
        action: 'visitedEmployerHomePage-DataEngineers',
      })
